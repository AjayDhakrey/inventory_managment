...
export default function ColorManagement({ business, account }) {
  ...
  return <section className="products-page color-management-page">
    ...
    <div className="products-controls"><div className="search-box"><span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="10.5" cy="10.5" r="6.5" /><path d="m16 16 4.5 4.5" /></svg></span><input aria-label="Search colors and products" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by color, product name or SKU..." /></div><span className="business-filter">{colors.length} colors</span></div>
