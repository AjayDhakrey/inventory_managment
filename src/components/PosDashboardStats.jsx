...
export default function PosDashboardStats({ business, onOpen }) {
  ...
  return <button className="pos-dashboard-strip" type="button" onClick={onOpen}><span><small>POS TODAY / HISTORY</small><strong>{data?.posInvoices || 0} bills</strong></span><span><small>POS SALES</small><strong>{business.currency} {Number(data?.posSalesTotal || 0).toFixed(2)}</strong></span><span><small>COLLECTED</small><strong>{business.currency} {Number(data?.posPaymentsReceived || 0).toFixed(2)}</strong></span><span className={data?.posOutstanding > 0 ? 'due' : ''}><small>OUTSTANDING</small><strong>{business.currency} {Number(data?.posOutstanding || 0).toFixed(2)}</strong></span><b>Open POS →</b></button>
}
