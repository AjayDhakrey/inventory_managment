...
export default function NotificationCenter({ onNavigate, onOpen, onChange }) {
  ...
  return <div className="header-popover-wrap" ref={popoverRef}>
    <button className={`icon-button${pulse ? ' notif-pulse' : ''}`} type="button" aria-label={`Notifications${data.unreadCount ? `, ${data.unreadCount} unread` : ''}`} aria-expanded={open} data-tooltip="Notifications" onClick={() => { setOpen(!open); onOpen?.(); if (!open) load() }}><svg className="header-action-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4" /></svg>{data.unreadCount > 0 && <span className="notification-badge">{data.unreadCount > 99 ? '99+' : data.unreadCount}</span>}</button>
